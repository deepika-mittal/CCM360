CREATE FUNCTION FUNC_GENERATE_ENVELOPE_ID
(
	@sortCode varchar(10),
...
AS
BEGIN
	declare @hashString varchar(32) = convert(varchar(32), hashbytes('MD5', concat(@concatAddress, @sortCode)), 2);
	RETURN concat(@batchDate, '_', @hashString);
END
GO


CREATE PROCEDURE SP_BUNDLE_POSTALADRS 
	@sortCode varchar(10),
	@batchDate varchar(15),
	@concatAddress varchar(500),
...
BEGIN
	...
	*/
	...
	declare PostalAddresses cursor local for select SORTCODE, STRSBATCHDATE, STRSCONCATADDRESS from TAB_STRSPOSTALADRS
		where STRSENVELOPEID = 'not initialized' and STRSBATCHDATE = @p_STRSBATCHDATE
		group by SORTCODE, STRSBATCHDATE, STRSCONCATADDRESS;

	open PostalAddresses;
	fetch next from PostalAddresses into @sortCode, @batchDate, @concatAddress;

	while @@FETCH_STATUS = 0
	begin
		exec SP_BUNDLE_POSTALADRS @sortCode, @batchDate, @concatAddress, @MAX_ENVELOPE_SHEETS, @MAX_ENVELOPES;
		fetch next from PostalAddresses into @sortCode, @batchDate, @concatAddress;
	end
	close PostalAddresses;
END
